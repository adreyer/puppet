...
class Puppet::Module
  class Task
    ...
    def self.get_file_details(path, name_start)
      ...
      name = name_start.partition('name_start').last
    ...
    def self.find_files(files, mod)
      ...
      file_list = files.flat_map do |file|
        ...
        if File.directory?(path)
          ...
          files = dir_files.map { |f| get_file_details(f, mod.name) }
        else
          files = get_file_details(path, endpath)
        end
    ...
    def files
      ...
      # PXP agent relies on 'impls' (which is the task file) being first
      task_file + lib_files
    end
